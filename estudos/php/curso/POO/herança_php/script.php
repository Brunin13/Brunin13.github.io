...
<body>
  ...
  class Pai {
    public function __construct(public string $nome="", public int $idade = 0)
    {
      $this->nome = $nome;
      $this->idade = $idade;
    }
    ...
    public function getNome(){
      prints("o nome é ".$this->nome);
    }

    protected function getIdd()
    {
      prints("a idade é " . $this->idade);
    }

    private function showAll() {
      echo "nome: ".$this->getNome." <br>idade: ".$this->getNome;
    }
  ...
  $pai = new Pai("Jurandir", 47);
  prints(var_dump($pai));
  $filho = new Filho("azuis", "Bruno", 11);
  prints(var_dump($filho));
  $filho->getNome();
  $pai->getNome();
  // $filho->showAll();
